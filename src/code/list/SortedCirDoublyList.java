...
public class SortedCirDoublyList<T extends Comparable<? super T>> extends CirDoublyList<T> {
	...
	public DoubleNode<T> search(T key) {
		for (DoubleNode<T> p = this.head.next; p != head
				&& key.compareTo(p.data) >= 0; p = p.next) {
	...
	private void quickSort(DoubleNode<T> begin, DoubleNode<T> end) {
		if (begin != end && begin != end.next) // 排除空和单结点链表情况
		{
			...
			System.out.println(
					begin.data + ".." + end.data + ",  vot=" + vot + "  " + this.toString());
	...
	public SortedCirDoublyList<T> mergeWith(SortedCirDoublyList<T> list) {
		...
		while (p != this.head || q != list.head) // 遍历两条排序循环双链表
			if (p != this.head
					&& (q != list.head && (p.data).compareTo(q.data) <= 0 || q == list.head)) { // 复制this结点，若p结点值小，或q已结束
				rear.next = new DoubleNode<T>(p.data, rear, null);
				rear = rear.next;
				p = p.next;
			} else if (q != list.head
					&& (p != this.head && (p.data).compareTo(q.data) > 0 || p == this.head)) { // 否则，复制list结点，若q结点值小，或p已结束
