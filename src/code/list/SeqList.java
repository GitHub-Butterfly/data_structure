...
public class SeqList<T> extends Object {
	...
	/*
	 * 可行，效率同上 public String toString() { String str="("; if (this.n()!=0) { for(int
	 * i=0; i<this.n()-1; i++) str += this.get(i).toString()+", "; str +=
	 * this.get(this.n()-1).toString(); } return str+")"; }
	 */
	...
	public T remove_Joseph_Ring(int i) {// 删除第i个元素，0≤i<n，返回被删除元素。若i越界，返回null。//？？若i越界，抛出序号越界异常
		if (this.n > 0 && i >= 0 && i < this.n) {
			T old = (T) this.element[i]; // old中存储被删除元素
			for (int j = i; j < this.n - 1; j++)
				this.element[j] = this.element[j + 1]; // 元素前移一个位置
	...
	// 5. 顺序表的浅拷贝与深拷贝
	/*
	 * public SeqList(SeqList<T> list) //浅拷贝构造方法 { this.n = list.n; //int整数赋值，复制了整数值
	...
	public SeqList(SeqList<? extends T> list) // 拷贝构造方法，深拷贝，复制list
	{
		...
		// this.element[i] = new Object(list.element[i]);
		// //语法错，因为Object没有提供拷贝构造方法，且构造方法不能继承
	}
	...
	/*
	 * //子类不支持 public void insert(int i,SeqList<T> list)
	 * //将list所有元素添加到当前顺序表最后//顺序表连接，首尾相接 { for (int j=0; j<list.n; j++)
